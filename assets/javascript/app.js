...
const questionData = [
    ...
    },
    {
        ...
        flavor: "A Gentleman and a Scholar",
...
function calculateScore() {
    ...
    for (var x = 0; x < questionData.length; x++) {
        if (questionData[x].UserAnswer !== "Nothing Selected" && questionData[x].UserAnswer.includes(questionData[x].answers[questionData[x].correctAnswerIndex]))
            score++;
    }
    return score;
}
function displayScore(score) {
    ...
    if (targetDiv)
        targetDiv.innerHTML = "You Scored " + score + " out of " + questionData.length;
}
;
function clickListener(event) {
    if (gameOver) {
        ...
        return;
    } // also need to display game over message, and clear interval
...
function populateAnswers() {
    ...
    if (targetDiv && targetDiv.classList.contains("invisible")) {
...
function showScores(x) {
    ...
    if (x.UserAnswer.includes(x.answers[x.correctAnswerIndex]))
        returnString += '<div class="alert alert-success" style="font-weight:bold;">';
    else
        returnString += '<div class="alert alert-danger" style="font-weight:bold;">';
...
function endGameClickListener(event) {
    if (gameOver) {
        var clickedValue = event.target;
        if (clickedValue.attributes[1] && clickedValue.attributes[1].value.includes("choice-btn") && typeof clickedValue.attributes[0] !== "undefined" && clickedValue.attributes[0].value === "show-answers") {
            populateAnswers();
            clickedValue.remove();
            targetDiv = document.getElementById("reload-page");
            if (targetDiv && targetDiv.classList.contains("col-md-6"))
                targetDiv.classList.replace("col-md-6", "col-md-12");
        }
    }
}
function nextQuestion() {
    if (questionIndex > 0 && typeof questionData[questionIndex - 1].UserAnswer === "undefined")
        questionData[questionIndex - 1].UserAnswer = textElements.selectedAnswerText.innerHTML; //this sets the users answer in the data object if the question times out instead of user clicking next question 
    ...
    if (questionIndex < questionData.length) {
        ...
        textElements.questionNumberText.textContent = "Question #" + (questionIndex + 1);
        ...
        gameOverFunc();
        return;
    } // call a game over function to calculate and display score } // display game over message and remove click listener and clear interval
    resetTimer();
}
function gameLoop() {
    nextQuestion();
