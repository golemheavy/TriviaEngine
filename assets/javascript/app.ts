...
function displayTime() {
	...
	if (targetDiv) targetDiv.innerHTML = time;
}
...
// unhide scoreboard div and hide question card / container div
	...
	if (targetDiv) targetDiv.innerHTML = "You Scored " + score + " out of " + questionData.length ; 
};
...
function endGameClickListener(event) {
	if (gameOver) {
		...
		if (clickedValue.attributes[1] && clickedValue.attributes[1].value.includes("choice-btn") && typeof clickedValue.attributes[0] !== "undefined" && clickedValue.attributes[0].value === "show-answers") {
			...
			if (targetDiv && targetDiv.classList.contains("col-md-6")) targetDiv.classList.replace("col-md-6","col-md-12");
		}
	}
}
